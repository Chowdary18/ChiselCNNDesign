...
object FPArithmetic extends App{
  ...
  class FP_adder(bw: Int) extends Module{
    require(bw == 16 || bw == 32 || bw == 64 || bw == 128) // the 4 precisions allowed
    ...
    var exponent = 0
    var mantissa = 0
    if (bw == 16){// get the exponent and mantissa for the precision specified
      exponent = 5
      mantissa = 10
    }else if (bw == 32){
      exponent = 8
    ...
    when(io.in_a(bw-2, mantissa) > BigInt(2).pow(exponent).U - 2.U){ // if input is too big, saturate to maximum value
    ...
    when( sub_exp >= mantissa.U){// if the difference between the exponents is too large, larger than mantissa size.
      new_s := out_s
      ...
      when(leadingOneFinder.io.out === 1.U && adder_result === 0.U && ((1.U === (sign(0) ^ sign(1)) && io.in_a(bw-2, 0)===io.in_b(bw-2,0)))){
        ...
        when(subber2.io.out_c === 1.U){
          ...
          new_out_frac := 0.U//BigInt(2).pow(mantissa-1).U(mantissa.W)
        }.otherwise {
  ...
  class FP_multiplier(bw: Int) extends Module{
    ...
    when(BigInt(2).pow(exponent).U - 2.U >= 1.U + ~(cond_holder) ){
      ...
      new_mant := 0.U//BigInt(2).pow(mantissa-1).U(mantissa.W)
...
//  (new chisel3.stage.ChiselStage).emitVerilog(new FP_adder(16))
}
